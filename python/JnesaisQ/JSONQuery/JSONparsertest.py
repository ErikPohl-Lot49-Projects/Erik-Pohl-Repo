...
class JSONParsertest(ut.TestCase):
    @pm.parameterized.expand([
        ...
        ("Basic mismatch", {'hello': '1'}, {'hello': 'x'}, MATCHMODES["MISMATCH"],
         [json_query_finding(current_json_path='/hello', actual_finding_value=JSON_VALUE_MISMATCH)]),
        ...
        ("Nested mismatch level 1 and 2", {'hello': '2', 'zap': {'h1': 'one', 'h2': 'two', 'single': '1'}},
         ...
         [json_query_finding(current_json_path='/hello', actual_finding_value=JSON_VALUE_MISMATCH),
          json_query_finding(current_json_path='/zap/single', actual_finding_value=JSON_VALUE_MISMATCH)]),

        ("Unexpected format key", {'hello': '1', 'zap': {'h1': 'one', 'h2': 'two', 'single': '1'}},
         {'hello': '.', 'blammo' : 'blam', 'zap': {'h1': 'one', 'h2': 'two', 'single': '.'}}, MATCHMODES["MISMATCH"],
         [json_query_finding(current_json_path='/blammo', actual_finding_value=JSON_KEY_MISSING)]),
        ...
        ("Nested mismatch with fewer keys in format", {'hello': '1', 'zap': {'h1': 'one', 'h2': 'two', 'single': '.'}},
         ...
         [json_query_finding(current_json_path='/zap/h1', actual_finding_value=JSON_VALUE_MISMATCH)]),
        ...
        ("report matches all ands satisfied with nesting",{'hello': '1', 'zap': {'h1': 'one', 'h2': 'two', 'single': '.'}},
         ...
         [json_query_finding(current_json_path='/zap/h1', actual_finding_value='one'),
          json_query_finding(current_json_path='/hello', actual_finding_value='1')]),
    ...
    def testJSONparser(self, testname, testinp, testinp2, matchtype, expected):
        ...
        #if testname == 'Nested mismatches with fewer keys in format':
        #    print(actual.json_query_finding)
