...
#TODO: refactor
#TODO: make delimiter logic better-- what if a comment or code has the delimiter in it naturally?
...
class deawkwardize:
    ...
    def __init__(self):
        ...
        self.standard_reawk_function_code_header = 'import logging, sys\nlogging.basicConfig(stream=sys.stdout, level=logging.INFO)\n'
    ...
    def deawk(self, deawkwardize_input_file_name, deawkwardize_output_file_name='deawked_',
        ...
        :param deawkwardize_output_file_name: Output deawkwardized python program
        ...
        def generate_deawkwardize_token(code_line_to_generate_a_token_for):
            ...
            try:
                return self.deawkwardize_token_dictionary[code_line_to_generate_a_token_for]
            except:
                self.current_token_sequence_number += 1
                self.deawkwardize_token_dictionary[code_line_to_generate_a_token_for] = self.current_token_sequence_number
                return (str(self.current_token_sequence_number))

        deawkwardize_output_file_name = deawkwardize_output_file_name + deawkwardize_input_file_name
        with open(deawkwardize_input_file_name, 'r') as deawk_input, open(deawkwardize_output_file_name, "w") as deawk_output, \
            ...
            for line in deawk_input:
                ...
                if line.strip().startswith('logging'):
                    newtoken = self.logging_token_prefix + str(generate_deawkwardize_token(line.strip()))
                    deawk_output.write(line.replace(line.strip(), newtoken) + '\n')
                    tokenfilehandle.write(newtoken + self.deawkwardize_token_file_delimiter + line.strip() + '\n')
                    continue
                if line.strip().startswith('#'):
                    newtoken = self.comment_token_prefix + str(generate_deawkwardize_token(line.strip()))
                    deawk_output.write(line.replace(line.strip(), newtoken) + '\n')
                    tokenfilehandle.write(newtoken + self.deawkwardize_token_file_delimiter + line.strip() + '\n')
                    continue
    ...
    def reawk_fileput(self, reawkwardize_input_file_name):
        ...
        :param reawkwardize_input_file_name: File name to output with comments and logging restored
        :return: True
        '''
        with open(reawkwardize_input_file_name, 'r') as reawkwardize_file_handle:
            for reawkwardize_file_line in reawkwardize_file_handle:
                ...
                if reawkwardize_file_line.strip().startswith('#'):
                    try:
                        print(reawkwardize_file_line.replace(reawkwardize_file_line.strip(),
                                                             self.reawkwardize_token_dictionary[reawkwardize_file_line.strip()]))
                    except:
    ...
    def reawk_logging(self, *argsx):
        ...
        def real_decorator(func):
            ...
            def wrapper(*args, **kwargs):
                ...
                function_source_code_lines = function_source_code.split('\n')
                reawked_replacement_source_code = self.standard_reawk_function_code_header
                for function_source_line in function_source_code_lines:
                    if function_source_line.strip().startswith(self.logging_token_prefix):
                        try:  # % only, though
                            reawk_code_line_replacement = self.reawkwardize_token_dictionary[function_source_line.strip()]
                            reawked_replacement_source_code += reawk_code_line_replacement + "\n"
                            continue
                        except:
                            reawk_code_line_replacement = function_source_line.strip()
                        reawked_replacement_source_code += reawk_code_line_replacement.strip() + '\n'
                    elif not function_source_line.strip().startswith('def') \
                            and not function_source_line.strip().startswith('@'):
                        reawked_replacement_source_code += function_source_line.strip() + '\n'
                # print("new innersource\n", newtestinnersource)
                code_obj = compile(reawked_replacement_source_code, '<string>', 'exec')
